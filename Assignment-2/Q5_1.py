...
final_output = Dense(n, activation = 'sigmoid')(encoder2_sa)
...
stack_autoencoder.fit(x_tr, x_tr, batch_size=batch_size, epochs=epochs, verbose=1)

elm_train_input = stack_autoencoder.predict(x_tr, batch_size=batch_size)
elm_test_input = stack_autoencoder.predict(x_ts, batch_size=batch_size)

p = x_tr.shape[1] + 100 # no of neurons in hidden layer
